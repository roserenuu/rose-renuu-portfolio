export function HeroSection() {
  return (
    <section
      style={{ backgroundColor: "#ecede8", minHeight: "882px" }}
      className="w-full flex items-stretch"
    >
      <div
        style={{
          padding: "min(4.4vw, 60px) min(5.86vw, 80px)",
          gap: "min(4.4vw, 60px)",
          maxWidth: "1366px",
          margin: "0 auto",
          width: "100%",
        }}
        className="flex flex-col md:flex-row w-full"
      >
        {/* LEFT COLUMN */}
        <div
          style={{ gap: "20px" }}
          className="flex flex-col items-center md:w-[45%] w-full"
        >
          {/* Portrait photo */}
          <img
            src="/images/rose-renuu-headshot.jpg"
            alt="Rose Renuu portrait"
            style={{
              width: "min(31.5vw, 430px)",
              height: "min(31.5vw, 430px)",
              objectFit: "cover",
              borderRadius: 0,
              maxWidth: "100%",
            }}
          />

          {/* Location */}
          <p
            style={{
              fontFamily: '"MelanieDisplay", serif',
              fontSize: "24px",
              color: "#60590c",
              fontWeight: 400,
              margin: 0,
            }}
          >
            Based in Los Angeles, CA
          </p>

          {/* Contact */}
          <p
            style={{
              fontFamily: '"MelanieDisplay", serif',
              fontSize: "27px",
              color: "#60590c",
              margin: 0,
              textAlign: "center",
            }}
          >
            Contact:{" "}
            <a
              href="mailto:roserenuu@gmail.com"
              style={{
                color: "#60590c",
                textDecoration: "underline",
              }}
            >
              roserenuu@gmail.com
            </a>
          </p>

          {/* Social icons row */}
          <div className="flex flex-row items-center" style={{ gap: "16px" }}>
            {/* TikTok */}
            <a
              href="https://www.tiktok.com/@roserenuu"
              target="_blank"
              rel="noopener noreferrer"
              style={{
                width: "80px",
                height: "80px",
                borderRadius: "12px",
                backgroundColor: "#704608",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
              }}
              aria-label="TikTok"
            >
              <svg
                viewBox="0 0 24 24"
                fill="white"
                style={{ width: "40px", height: "40px" }}
              >
                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V9.83a8.21 8.21 0 004.79 1.54V7.93a4.86 4.86 0 01-1.02-.24z" />
              </svg>
            </a>

            {/* Instagram */}
            <a
              href="https://www.instagram.com/roserenuu"
              target="_blank"
              rel="noopener noreferrer"
              style={{
                width: "80px",
                height: "80px",
                borderRadius: "12px",
                backgroundColor: "#704608",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
              }}
              aria-label="Instagram"
            >
              <svg
                viewBox="0 0 24 24"
                fill="white"
                style={{ width: "40px", height: "40px" }}
              >
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
              </svg>
            </a>

            {/* X / Twitter */}
            <a
              href="https://x.com/roserenuu"
              target="_blank"
              rel="noopener noreferrer"
              style={{
                width: "80px",
                height: "80px",
                borderRadius: "12px",
                backgroundColor: "#704608",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
              }}
              aria-label="X (Twitter)"
            >
              <svg
                viewBox="0 0 24 24"
                fill="white"
                style={{ width: "40px", height: "40px" }}
              >
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.261 5.634 5.903-5.634zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
          </div>
        </div>

        {/* RIGHT COLUMN */}
        <div
          style={{ paddingTop: "80px", gap: "16px" }}
          className="flex flex-col md:w-[55%] w-full"
        >
          {/* Meet Melanie heading */}
          <h1
            style={{
              fontFamily: '"MelanieDisplay", serif',
              fontSize: "min(6.3vw, 86px)",
              fontWeight: 400,
              color: "#704608",
              letterSpacing: "-0.04em",
              lineHeight: 1,
              margin: 0,
            }}
          >
            Meet Rose
          </h1>

          {/* UGC CREATOR label */}
          <p
            style={{
              fontFamily: '"MelanieSans", ui-sans-serif, system-ui, sans-serif',
              fontSize: "min(2.34vw, 32px)",
              fontWeight: 700,
              color: "#704608",
              margin: 0,
            }}
          >
            UGC CREATOR
          </p>

          {/* Bullet list */}
          <ul
            style={{
              fontFamily: '"MelanieSans", ui-sans-serif, system-ui, sans-serif',
              fontSize: "min(2.34vw, 32px)",
              color: "#383035",
              fontWeight: 400,
              margin: 0,
              paddingLeft: "24px",
            }}
          >
            <li>Gen Z creator (26 years old)</li>
            <li>Avid fitness enthusiast</li>
            <li>Coffee lover ☕</li>
            <li>Mental health advocate</li>
            <li>Speaks English and basic Mandarin</li>
          </ul>

          {/* Strategic partner paragraph */}
          <p
            style={{
              fontFamily: '"MelanieSans", ui-sans-serif, system-ui, sans-serif',
              fontSize: "min(1.61vw, 22px)",
              color: "#383035",
              textAlign: "center",
              margin: 0,
            }}
          >
            {"You're not paying for videos alone, you're paying for a "}
            <strong>strategic creative partner</strong>
            {" focused on "}
            <strong>performance and results.</strong>
          </p>

          {/* Stats images */}
        </div>
      </div>
    </section>
  );
}
